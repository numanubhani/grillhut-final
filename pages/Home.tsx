            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredProducts.map(product => {
                ...
                return (
                  <div 
                    key={product.id}
                    className="group bg-white dark:bg-zinc-900 rounded-[2rem] overflow-hidden border border-zinc-200/80 dark:border-zinc-800/80 hover:border-orange-500/50 dark:hover:border-orange-500/30 transition-all duration-300 hover:shadow-2xl hover:shadow-orange-500/10 hover:-translate-y-1"
                  >
                    <div 
                      onClick={() => {
                    ...
                    >
                      <div className="relative aspect-[4/3] overflow-hidden bg-zinc-100 dark:bg-zinc-800">
                        ...
                        {(product.discount || discount > 0) && (
                          <div className="absolute top-3 left-3 bg-red-600 text-white px-3 py-1.5 rounded-lg font-black text-xs shadow-lg">
                            <div>{product.discount || discount}% OFF</div>
                            {product.discountText && (
                              <div className="text-[9px] font-normal mt-0.5 leading-tight">{product.discountText}</div>
                            )}
                          </div>
                        )}
                        <button
                          ...
                          className="absolute top-3 right-3 w-9 h-9 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-sm rounded-full flex items-center justify-center hover:scale-110 transition-transform shadow-lg z-10"
                        >
                          <Heart className={`w-4 h-4 ${isLiked ? 'fill-red-500 text-red-500' : 'text-zinc-400'}`} />
                        </button>
                        <div className="absolute bottom-3 right-3 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-sm px-2 py-1 rounded-lg flex items-center gap-1 shadow-lg">
                      ...
                      <div className="p-5">
                        <h3 className="text-lg font-black font-serif leading-tight mb-2 text-zinc-900 dark:text-white group-hover:text-orange-600 transition-colors">
                          {product.name}
                        </h3>
                        <p className="text-sm text-zinc-500 dark:text-zinc-400 line-clamp-2 mb-4 font-medium leading-relaxed min-h-[2.5rem]">
                          {product.description}
                        </p>
                        
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-baseline gap-2">
                            <span className="text-xl font-black text-orange-600">Rs. {finalPrice.toFixed(2)}</span>
                            {product.originalPrice && (
                              <span className="text-xs text-zinc-400 line-through">Rs. {product.originalPrice.toFixed(2)}</span>
                            )}
                          </div>
                        </div>
                        ...
                        <button
                          ...
                          className="w-full flex items-center justify-center gap-2 py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold text-sm transition-all active:scale-95 shadow-md shadow-red-600/30"
                        >
